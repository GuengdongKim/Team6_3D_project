...
public class FireController : MonoBehaviour
{
    ...
    public float time = 10.0f; // 타이머
    public float fireHp = 10.0f; // 불 체력
    public float PlayerHp = 100.0f; // 플레이어 체력
    ...
    {
        if (other.gameObject.tag == "fe" || other.gameObject.tag == "Door" || other.gameObject.tag == "Player") // 물, 문, 플레이어는 불 삭제 제외
        {
            
        }
        else // 그 외 불, 벽 등에 접촉시 제거
        {
            Destroy(this.gameObject);
        }
    }
